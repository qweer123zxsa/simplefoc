...
void setPhaseVoltage(float Uq, float Ud, float angle_el)
{
	...
	{// _sqrt is an approx of sqrt (3-4% error)
		// 计算合成电压矢量幅值，并归一化到电源电压
		Uout = _sqrt(Ud*Ud + Uq*Uq) / voltage_power_supply;
		// angle normalisation in between 0 and 2pi
		// only necessary if using _sin and _cos - approximation functions
		// 计算合成电压矢量的相位角，并进行归一化处理
		// atan2(Uq, Ud) 计算电压矢量相对于d轴的角度
		angle_el = _normalizeAngle(angle_el + atan2(Uq, Ud));
	}
	else
	{// only Uq available - no need for atan2 and sqrt
		// 当只有Uq时（常见于只需控制转矩的情况），直接计算幅值
		Uout = Uq / voltage_power_supply;
		// angle normalisation in between 0 and 2pi
		// only necessary if using _sin and _cos - approximation functions
		// Q轴超前D轴90度（PI/2），直接加上即可
		angle_el = _normalizeAngle(angle_el + _PI_2);
	}
	// SVPWM 最大调制比限制 (1/sqrt(3) ≈ 0.577)
	// 限制电压矢量幅值在六边形内切圆内，保证线性调制区
	if(Uout> 0.577)Uout= 0.577;
	if(Uout<-0.577)Uout=-0.577;
	
	// 计算扇区号 (1-6)
	// 将电角度除以60度 (PI/3) 确定所在扇区
	sector = (angle_el / _PI_3) + 1;
	
	// SVPWM 核心算法：计算相邻两个基本电压矢量的作用时间 T1, T2
	// 使用正弦定理计算
	...
	// 根据扇区分配三相占空比 (Ta, Tb, Tc)
	// 这里采用七段式SVPWM的变种，将零矢量时间均匀分配
	switch(sector)
	{
		case 1:
			Ta = T1 + T2 + T0/2;
